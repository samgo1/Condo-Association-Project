...
<A HREF="./views/groupcreate.php?" onClick=" return popup(this, 'notes')">Click here to add a new group</A>
...
$SQLcommand = 'SELECT CONCAT(\'ID=\',g.id) as group_ID, g.name as group_name, g.owner as group_owner_ID, m.name as owner_name, CONCAT(\'delgr=\',g.id) as del
...
while ($row = mysqli_fetch_array($result)) {
    ...
    foreach ($all_property as $item) {
        ...
        {   $ID = substr($row[$item], 3);
            echo "<td> <A HREF=\"./views/groupsedit.php?ID=".$ID."\" onClick=\" return popup(this, 'notes')\">".$ID."</A> </td>";
        }
        else {
               if (substr($row[$item], 0, 6) == 'delgr=')
                {   $ID = substr($row[$item], 6);

                   echo "<td> <A HREF=\"./views/groupdelete.php?ID=".$ID."\" onClick=\" return popup(this, 'notes')\">"."del"."</A> </td>";
                 }
